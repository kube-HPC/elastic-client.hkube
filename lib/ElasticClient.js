...
class ElasticClient extends EventEmitter {
    constructor(options) {
        ...
        this.options.auth = options.token ? {
    ...
    createIndex(options) {
        return new Promise((resolve, reject) => {
            this.client.indices.create({
                ...
                return resolve({ exists: false, response });
            }, (error) => {
                return reject(error);
            });
        });
    ...
    _formatResponse(response) {
        return {
            ...
            hits: this._mapAllHits(response.body.hits.hits)
        };
    }
    ...
    _mapMultiSearch(responses) {
        ...
        for (const r of responses) {
            if (r.error) {
                sources.push({ error: r.error.type, index });
            }
                ...
                sources.push({ error: new Error('empty response'), index });
            }
            index += 1;
        }
        return sources;
    ...
    _resolvePing(error) {
        ...
        else if (!this.isActive || this.isInit) {
            this.isInit = false;
            this.isActive = true;
